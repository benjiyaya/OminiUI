...
def api_model_unload():
    """Release GPU memory by unloading the HiDream model."""
    with _GEN_LOCK:
    ...
    print("[app] Model unloaded, GPU memory released")
    return jsonify({"status": "unloaded"})


@app.route("/api/model/reload", methods=["POST"])
def api_model_reload():
    """Reload the HiDream model (requires --model_path to have been set at startup)."""
    model_path = _STATE.get("model_path") or os.environ.get("HIDREAM_MODEL_PATH")
    if not model_path:
        return jsonify({"error": "HIDREAM_MODEL_PATH not set"}), 400
    with _GEN_LOCK:
        if _STATE["model"] is not None:
        ...
        _STATE["model"] = model
    print("[app] Model reloaded")
    return jsonify({"status": "reloaded"})


@app.route("/api/model/status", methods=["GET"])
def api_model_status():
    return jsonify({"loaded": _STATE["model"] is not None})
